...
export function InventoryDashboard() {
  ...
  const trendOption = {
    ...
    series: [
      {
        name: translate("inventory.dashboard.trend.received", { ns: "starter" }, "Received"),
        type: "line",
        smooth: true,
        showSymbol: false,
      ...
      {
        name: translate("inventory.dashboard.trend.issued", { ns: "starter" }, "Issued"),
        type: "line",
        smooth: true,
        showSymbol: false,
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard
          ...
          label={translate("inventory.dashboard.kpi.activeProducts", { ns: "starter" }, "Active products")}
          value={String(activeCount)}
          sub={translate(
            "inventory.dashboard.kpi.activeProducts.sub",
            { ns: "starter", count: products.result.data.length },
            `${products.result.data.length} total in catalog`
          )}
        />
        <KpiCard
          loading={loading}
          ...
          label={translate("inventory.dashboard.kpi.unitsOnHand", { ns: "starter" }, "Units on hand")}
          value={formatNumber(totalUnits, locale)}
          sub={translate("inventory.dashboard.kpi.unitsOnHand.sub", { ns: "starter" }, "Across all warehouses")}
        />
        <KpiCard
          loading={loading}
          icon={DollarSign}
          tone="text-emerald-600 bg-emerald-500/12 dark:text-emerald-400"
          label={translate("inventory.dashboard.kpi.stockValue", { ns: "starter" }, "Stock value")}
          value={formatCurrency(totalValue, locale)}
          sub={translate("inventory.dashboard.kpi.stockValue.sub", { ns: "starter" }, "On-hand units × unit price")}
        />
        <KpiCard
          loading={loading}
          icon={AlertTriangle}
          tone="text-red-600 bg-red-500/12 dark:text-red-400"
          label={translate("inventory.dashboard.kpi.lowStock", { ns: "starter" }, "Low-stock items")}
          value={String(lowStock.length)}
          sub={translate("inventory.dashboard.kpi.lowStock.sub", { ns: "starter" }, "At or below reorder level")}
        />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>
              {translate("inventory.dashboard.stockByProduct.title", { ns: "starter" }, "Stock level by product")}
            </CardTitle>
            <CardDescription>
              {translate(
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              {translate("inventory.dashboard.lowStock.title", { ns: "starter" }, "Low-stock alerts")}
            </CardTitle>
            <CardDescription>
              {translate(
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-xs text-muted-foreground">
                        {e.product.sku || "—"} ·{" "}
                        {labelFor(CATEGORIES, e.product.category, translate)}
                      </p>
                    </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {translate("inventory.dashboard.trend.title", { ns: "starter" }, "Stock movement trend")}
          </CardTitle>
          <CardDescription>
            {translate(
              "inventory.dashboard.trend.description",
              { ns: "starter", weeks: WEEKS_BACK },
